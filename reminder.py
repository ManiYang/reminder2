...
class Reminder:
    """
    Attributes:
        _id: The ID of the reminder in the database. It can be only set once (via property `id`).
             It will be set by dataaccess.add_reminder(). Do not set it by other means.
        category_id: int or None
        content (str): Textual content of the reminder.
        act_time_model (ActTimeModel): Defines when the reminder shows up.
    """
    ...
    def to_dict(self, exclude_id=True):
        ...
        field_values = {'category_id': self.category_id,
                        ...
                        'act_time_model': self.act_time_model.to_json()}
        if not exclude_id:
